...
class qieti_tigan(data.Dataset):
  num_classes = 1
  ...
  def __init__(self, opt, split):
    super(qieti_tigan, self).__init__()
    self.data_dir = os.path.join(opt.data_dir, 'qieti_tigan')
    self.img_dir = os.path.join(self.data_dir, split)
    self.annot_path = os.path.join(
          self.data_dir, 'annotations', 'instances_%s.json' %split)
    self.max_objs = 128
    self.class_name = ['__background__', 'tigan']
    self._valid_ids = [1,]
    ...
    print('==> initializing Qieti_Tigan {} data.'.format(split))
