...
class bbk_formula(data.Dataset):
  num_classes = 1
  ...
  def __init__(self, opt, split):
    super(bbk_formula, self).__init__()
    self.data_dir = os.path.join(opt.data_dir, 'bbk_formula')
    self.img_dir = os.path.join(self.data_dir, split)
    self.annot_path = os.path.join(
          self.data_dir, 'annotations', 'instances_%s.json' %split)
    self.max_objs = 128
    self.class_name = ['__background__', 'formula']
    self._valid_ids = [1]
    ...
    print('==> initializing BBK Formula {} data.'.format(split))
